...
<form class="flex flex-row justify-center {$$restProps.class}">
	{#each models as currentModel, promptIdx}
		<div class="m-1 flex flex-1 w-12 h-12">
			...
			<label
				for={'model-' + promptIdx}
				class="inline-block rounded-full overflow-hidden w-12 h-12 border-2 border-transparent hover:bg-black/5 hover:text-ionos dark:hover:bg-white/5 transition"
			>
